...
import type { FriendDto, StudySessionDto, UserProfile } from '../types';
...
export default function Schedule() {
  ...
  const [editingSession, setEditingSession] = useState<StudySessionDto | null>(null);
              ...
              <div className="empty-state">
                ...
                <h3>
                  {tab === 'upcoming'
                      ...
                      : tab === 'mine'
                          ? "You haven't joined any sessions"
                          : "You haven't hosted any sessions"}
                </h3>
                <p>
                  {tab === 'upcoming'
                ...
                return (
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    {session.isHost ? 'You' : session.host.fullName}
                          {session.isHost && (
                              <span className="chip chip-primary" style={{ marginLeft: 6, fontSize: 10, padding: '1px 6px' }}>
                        Host
                      </span>
                          )}
                  </span>
...
function CreateSessionModal({
  ...
  const [loadingFriends, setLoadingFriends] = useState(!isEditing);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        invitedFriendIds: isEditing ? [] : selectedFriendIds,
      };
      ...
      setError(err.response?.data?.message || `Failed to ${isEditing ? 'update' : 'create'} session`);
    } finally {
      setLoading(false);
    }
  };
            ...
            {!isEditing && (
                <div className="form-group">
                  ...
                  {loadingFriends ? (
                      ...
                      <p className="text-sm text-muted">You do not have any connections to invite yet.</p>
                  ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {friends.map((friend) => {
                          const friendId = friend.user.id;
                          const selected = friendId ? selectedFriendIds.includes(friendId) : false;

                          return (
                              <label
                                  key={friendId ?? friend.user.email ?? friend.user.fullName}
                          ...
                          <span style={{ display: 'block', fontWeight: 600 }}>
                            {friend.user.fullName}
                                  ...
                                  {friend.sharedCourses.length > 0 && (
                                      <span className="text-sm text-muted">
                              Shared courses: {friend.sharedCourses.join(', ')}
                            </span>
                                  )}
                        </span>
                              </label>
